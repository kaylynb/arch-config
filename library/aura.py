...
# Forked from pacman package: https://raw.githubusercontent.com/ansible/ansible-modules-extras/devel/packaging/os/pacman.py
...
# Upgrade package foo
- aura: name=foo state=latest update_cache=yes
...
# Run the equivalent of "aura -Sy" as a separate step
- aura: update_cache=yes

# Run the equivalent of "aura -Su" as a separate step
- aura: upgrade=yes

# Run the equivalent of "aura -Syu" as a separate step
- aura: update_cache=yes upgrade=yes

# Run the equivalent of "aura -Rdd", force remove package baz
- aura: name=baz state=absent force=yes
'''
...
def upgrade(module, pacman_path):
    cmdupgrade = "%s -Aauq --noconfirm" % (pacman_path)
    cmdneedrefresh = "%s -Aaqu" % (pacman_path)
...
def main():
    ...
    pacman_path = module.get_bin_path('aura', True)
